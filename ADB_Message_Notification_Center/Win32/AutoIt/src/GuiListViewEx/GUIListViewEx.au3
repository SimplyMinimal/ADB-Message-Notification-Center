...
Func _GUIListViewEx_EditHeader($iLV_Index = Default, $iCol = Default, $iDelta_X = 0, $iDelta_Y = 0)
	...
	_GUIListViewEx_GetLVCoords($hLV_Handle, $tPoint)
	...
	Local $aEdit_Coords = _GUIListViewEx_EditCoords($hLV_Handle, $cLV_CID, $aLocation, $tPoint, $aLVClient[0] - 5, $iDelta_X, $iDelta_Y)
...
Func _GUIListViewEx_WM_LBUTTONUP_Handler($hWnd, $iMsg, $wParam, $lParam)
	...
	If $hGLVEx_SrcHandle = $hGLVEx_TgtHandle Then
		...
		_GUIListViewEx_ReWriteLV($hGLVEx_SrcHandle, $cGLVEx_SrcID, $aGLVEx_SrcArray, $aCheck_Array, $iGLVEx_SrcIndex)
	...
	Else ; Dropping in another ListView
		...
		_GUIListViewEx_ReWriteLV($hGLVEx_SrcHandle, $cGLVEx_SrcID, $aGLVEx_SrcArray, $aGLVEx_SrcArray, $iGLVEx_SrcIndex, False)
		_GUIListViewEx_ReWriteLV($hGLVEx_TgtHandle, $cGLVEx_TgtID, $aGLVEx_TgtArray, $aGLVEx_TgtArray, $iGLVEx_TgtIndex, False)
...
Func _GUIListViewEx_EditProcess($iLV_Index, $aLocation, $sCols, $iDelta_X, $iDelta_Y, $iEditMode)
	...
	Local $hTemp_Edit, $iKey_Code, $iCombo_State, $aSplit, $sInsert, $fClick_Move = False, $fCursor_Move = True
	...
	Local $tPoint = DllStructCreate("struct;long X;long Y;endstruct")
	; Get position of ListView within GUI client area
	_GUIListViewEx_GetLVCoords($hGLVEx_Editing, $tPoint)
	; Get ListView client area to allow for scrollbars
	...
	While 1
		...
		Local $aEdit_Pos = _GUIListViewEx_EditCoords($hGLVEx_Editing, $cEditingID, $aLocation, $tPoint, $aLVClient[0] - 5, $iDelta_X, $iDelta_Y)
		...
		If $fCombo Then
			...
			Else
				$cGLVEx_EditID = GUICtrlCreateCombo("", $aEdit_Pos[0], $aEdit_Pos[1], $aEdit_Pos[2], $aEdit_Pos[3], 0x02) ; $CBS_DROPDOWN
			EndIf
			GUICtrlSetFont($cGLVEx_EditID, $aLV_FontDetails[0], Default, Default, $aLV_FontDetails[1])
		...
		While 1
			If $fCombo Then
				; Check for dropdown opena and close
					...
					If $fReset_Edits Then
						...
						For $i = $aEdited[0][0] To 1 Step -1
							...
							Switch UBound($aGLVEx_SrcArray, 0)
								Case 1
									$aSplit = StringSplit($aGLVEx_SrcArray[$aEdited[$i][0] + 1], "|")
...
; Modified ......:
; ===============================================================================================================================
Func _GUIListViewEx_EditCoords($hLV_Handle, $cLV_CID, $aLocation, $tPoint, $iLVWidth, $iDelta_X, $iDelta_Y)
	...
	$aEdit_Data[0] = DllStructGetData($tPoint, "X") + $aRect[0] + $iDelta_X + 2
	$aEdit_Data[1] = DllStructGetData($tPoint, "Y") + $aRect[1] + $iDelta_Y
...
; Modified ......:
; ===============================================================================================================================
Func _GUIListViewEx_ReWriteLV($hLVHandle, $cLV_CID, ByRef $aLV_Array, ByRef $aCheck_Array, $iLV_Index, $fCheckBox = True)

	Local $iVertScroll, $sLine, $iNewItem
	...
	; Check if native or UDF ListView
	If $cLV_CID Then
		; Delete native created items
		For $i = _GUICtrlListView_GetItemCount($hLVHandle) - 1 To 0 Step -1
			GUICtrlDelete(_GUICtrlListView_GetItemParam($hLVHandle, $i))
		Next
		; Refill items from array
		For $i = 1 To $aLV_Array[0][0]
			; Create item text
			$sLine = ""
			For $j = 0 To UBound($aLV_Array, 2) - 1
				$sLine &= $aLV_Array[$i][$j] & "|"
			Next
			; Create item
		...
		Next
	Else
		; Delete UDF created items
		DllCall("user32.dll", "lresult", "SendMessageW", "hwnd", $hLVHandle, "uint", $LVM_DELETEALLITEMS, "wparam", 0, "lparam", 0)
		; Refill items from array
		For $i = 1 To $aLV_Array[0][0]
			; Add Item
			$iNewItem = _GUICtrlListView_AddItem($hLVHandle, $aLV_Array[$i][0])
			; Add new subitems
			For $j = 1 To UBound($aLV_Array, 2) - 1
				_GUICtrlListView_AddSubItem($hLVHandle, $iNewItem, $aLV_Array[$i][$j], $j)
			Next
...
; Modified ......:
; ===============================================================================================================================
Func _GUIListViewEx_GetLVCoords($hLV_Handle, ByRef $tPoint)
...
Func _GUIListViewEx_GetCursorWnd()
	...
	Local $aHwnd = DllCall("user32.dll", "hwnd", "WindowFromPoint", "struct", $tPoint)
	Return $aHwnd[0]
